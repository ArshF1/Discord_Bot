...
async def on_message(message):
    ...
    elif message.content.lstrip().startswith("-check"):
        if OS=="nt":
            code=subprocess.check_output(f'''python -c "{message.content[7:]}" ''',shell=True,text=True)
            await message.channel.send(code)
            # await message.channel.send(f"Code ended with exit code {code}")
            
        elif OS=="posix":
            code=os.system(f'''python3 -c "{message.content[7:]}"''')
            await message.channel.send(f"Code ended with exit code {code}")
